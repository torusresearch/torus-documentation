...
>Solana Redirect Flow is a mechanism with which dapps can support Torus Solana Wallet without adding external libraries in the existing code base. Not only it does it enable to get running in no time, the lean code is easily maintainable.
